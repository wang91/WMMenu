...
typedef enum {
    ...
    WMMenuViewArrowDirectionNone,
    WMMenuViewArrowDirectionUp,
    WMMenuViewArrowDirectionDown,
    WMMenuViewArrowDirectionLeft,
    WMMenuViewArrowDirectionRight,
    
} WMMenuViewArrowDirection;
...
@interface WMMenuOverlay : UIView
...
#pragma mark - WMMenuOverlay

@implementation WMMenuOverlay
...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[WMMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]) {
...
@implementation WMMenuView {
    WMMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView *_contentView;
    NSArray *_menuItems;
    WMMenuSelectedItem _selectedItem;
}
...
- (id)init {
    ...
    if(self) {
        ...
        if ([WMMenu hasShadow]) {
            ...
            self.layer.shadowColor = [WMMenu tintColor].CGColor;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect {
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) {
        ...
        _arrowDirection = WMMenuViewArrowDirectionUp;
        ...
        _arrowDirection = WMMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = WMMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
        ...
        _arrowDirection = WMMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
        ...
        _arrowDirection = WMMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(WMMenuSelectedItem)selectedItem {
    ...
    WMMenuOverlay *overlay = [[WMMenuOverlay alloc] initWithFrame:view.bounds];
    ...
    if (self.superview) {
        if (animated) {
            ...
            [UIView animateWithDuration:0.2 animations:^(void) {
                ...
                if ([self.superview isKindOfClass:[WMMenuOverlay class]])
...
- (void)performAction:(id)sender {
    ...
    WMMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *) mkContentView {
    ...
    UIFont *titleFont = [WMMenu titleFont];
    
    CGFloat maxImageWidth = 0;
    CGFloat maxItemHeight = 0;
    CGFloat maxItemWidth = 0;
    
    for (WMMenuItem *menuItem in _menuItems) {
    ...
    UIImage *selectedImage = [WMMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    UIImage *gradientLine = [WMMenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == WMMenuViewArrowDirectionUp) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };
        
    } else if (_arrowDirection == WMMenuViewArrowDirectionDown) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };
        
    } else if (_arrowDirection == WMMenuViewArrowDirectionLeft) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };
        
    } else if (_arrowDirection == WMMenuViewArrowDirectionRight) {
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    UIColor *tintColor = [WMMenu tintColor];
    if (tintColor) {
        CGFloat a;
    ...
    if ([WMMenu backgrounColorEffect] == WMMenuBackgrounColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == WMMenuViewArrowDirectionLeft ||
        _arrowDirection == WMMenuViewArrowDirectionRight) {
...
/// 背景色效果
static WMMenuBackgrounColorEffect   gBackgroundColorEffect = WMMenuBackgrounColorEffectSolid;
/// 是否显示阴影
...
+ (void)reset {
    gTintColor = nil;
    gTitleFont = nil;
    gBackgroundColorEffect = WMMenuBackgrounColorEffectSolid;
    gHasShadow = NO;
}
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(WMMenuSelectedItem)selectedItem {
    ...
    // 创建MenuView
    _menuView = [[WMMenuView alloc] init];
